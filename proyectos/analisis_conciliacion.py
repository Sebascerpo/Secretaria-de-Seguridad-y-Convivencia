...
def load_data(filepath):
    ...
    try:
        ...
        # Convertir fechas
        date_cols = [
            "fecha_estado_negocio",
            "fecha_inicio_evento",
            "fecha_fin_evento",
            "fecha_inicio",
            "fecha_entrega_auxiliar",
        ]
        ...
        df["tiene_acuerdo"] = df["evento"].str.contains(
            "ACUERDO CONCILIATORIO", case=False, na=False
        )
        df["tiene_no_acuerdo"] = df["evento"].str.contains(
            "NO ACUERDO", case=False, na=False
        ...
        df["tiene_inasistencia"] = (
            ...
            | df["evento"].str.contains(
                "CONTANCIA DE NO REALIZACION", case=False, na=False
            )
        ...
        if mask_tiempo.any():
            fecha_inicio_evento = remove_timezone(
                df.loc[mask_tiempo, "fecha_inicio_evento"]
            )
            fecha_inicio = remove_timezone(df.loc[mask_tiempo, "fecha_inicio"])
            df.loc[mask_tiempo, "dias_procesamiento"] = (
        ...
        mask_duracion = (
            df["fecha_fin_evento"].notna() & df["fecha_inicio_evento"].notna()
        )
        if mask_duracion.any():
            fecha_fin = remove_timezone(df.loc[mask_duracion, "fecha_fin_evento"])
            fecha_inicio_evt = remove_timezone(
                df.loc[mask_duracion, "fecha_inicio_evento"]
            )
            df.loc[mask_duracion, "dias_duracion_evento"] = (
                fecha_fin - fecha_inicio_evt
            ).dt.days
...
def run(project_info):
    ...
    data_path = os.path.join(
        current_dir, "..", project_info.get("archivo_datos", "data/conciliaciones2.csv")
    )
    ...
    with st.sidebar:
        ...
        lista_responsables = ["TODOS"] + sorted(
            [
                r
                for r in df["responsable"].unique().tolist()
        ...
        lista_estados = ["TODOS"] + sorted(
            [
                e
                for e in df["estado"].unique().tolist()
        ...
        lista_estados_negocio = ["TODOS"] + sorted(
            [
                e
                for e in df["estado_negocio"].unique().tolist()
                if pd.notna(e) and e != "" and e != "nan"
            ]
        )
        ...
        lista_eventos = ["TODOS"] + sorted(
            [
                e
                for e in df["evento"].unique().tolist()
                if pd.notna(e) and e != "" and e != "nan"
            ]
        )
    ...
    if filtro_estado_negocio != "TODOS":
        df_filtrado = df_filtrado[
            df_filtrado["estado_negocio"] == filtro_estado_negocio
        ]
    ...
    total_casos_unicos = (
        df_filtrado["id_caso"].nunique() if "id_caso" in df_filtrado.columns else 0
    )

    # Contar acuerdos, no acuerdos, inasistencias (a nivel de caso único)
    df_casos = (
        df_filtrado.groupby("id_caso")
        .agg(
            {
                "tiene_acuerdo": "any",
    ...
    tasa_acuerdos = (
        (casos_con_acuerdo / total_casos_unicos * 100) if total_casos_unicos > 0 else 0
    )
    responsables_unicos = df_filtrado["responsable"].nunique()

    # Tiempo promedio de procesamiento
    tiempo_promedio = (
        df_filtrado["dias_procesamiento"].mean()
        if df_filtrado["dias_procesamiento"].notna().any()
        else 0
    )
    ...
    with tab1:
        ...
        with col1:
            ...
            df_responsable_summary = (
                df_filtrado.groupby("responsable")
                .agg({"id_caso": "nunique", "tiene_acuerdo": "sum"})
                .reset_index()
            )
            df_responsable_summary.columns = [
                "Responsable",
                "Total Casos",
                "Eventos con Acuerdo",
            ]
            df_responsable_summary = df_responsable_summary.sort_values(
                "Total Casos", ascending=False
            ).head(10)
            st.dataframe(
                df_responsable_summary, use_container_width=True, hide_index=True
            )

        with col2:
            st.markdown("#### Top 10 Eventos")
            df_evento_summary = (
                df_filtrado.groupby("evento").size().reset_index(name="Total")
            )
            df_evento_summary = df_evento_summary.sort_values(
        ...
        with col3:
            ...
            df_estado_summary = (
                df_filtrado.groupby("estado").size().reset_index(name="Total")
            )
        ...
        with col4:
            ...
            df_estado_negocio_summary = (
                df_filtrado.groupby("estado_negocio").size().reset_index(name="Total")
            )
            df_estado_negocio_summary = df_estado_negocio_summary.sort_values(
                "Total", ascending=False
            )
            ...
            fig_estado_negocio.update_traces(
        ...
        mes_max = (
            df_filtrado.groupby("mes_ano").size().idxmax()
            if "mes_ano" in df_filtrado.columns and not df_filtrado.empty
            else "N/A"
        )
        mes_max_count = (
            df_filtrado.groupby("mes_ano").size().max()
            if "mes_ano" in df_filtrado.columns and not df_filtrado.empty
            else 0
        )
        responsable_max = (
            df_filtrado.groupby("responsable").size().idxmax()
            if not df_filtrado.empty
            else "N/A"
        )
        responsable_max_count = (
            df_filtrado.groupby("responsable").size().max()
            if not df_filtrado.empty
            else 0
        )
    ...
    with tab2:
        ...
        df_casos_detalle = df_casos_detalle.merge(
            df_estado_caso[["id_caso", "estado_negocio", "tiene_retiro"]],
            on="id_caso",
            how="left",
        )
        ...
        mask_en_proceso = (
            (
                df_casos_detalle["estado_negocio"].str.contains(
                    "Vigente|Trámite", case=False, na=False
                )
            )
        ...
        df_casos_detalle.loc[df_casos_detalle["tiene_inasistencia"], "resultado"] = (
            "Inasistencia"
        )

        # Sin Acuerdo (sobrescribe Inasistencia, En Proceso y Retirado)
        df_casos_detalle.loc[df_casos_detalle["tiene_no_acuerdo"], "resultado"] = (
            "Sin Acuerdo"
        )

        # Con Acuerdo (prioridad máxima - sobrescribe todo)
        df_casos_detalle.loc[df_casos_detalle["tiene_acuerdo"], "resultado"] = (
            "Con Acuerdo"
        )

        # Resumen por resultado
        df_resultado = (
            df_casos_detalle.groupby("resultado")
            .agg({"id_caso": "count", "dias_procesamiento": "mean"})
            .reset_index()
        )
        ...
        df_resultado["Porcentaje"] = (
            df_resultado["Total Casos"] / df_resultado["Total Casos"].sum() * 100
        ).round(2)
        ...
        if "mes_ano" in df_filtrado.columns:
            ...
            df_mes_acuerdo["Tasa Acuerdos (%)"] = (
                df_mes_acuerdo["Con Acuerdo"] / df_mes_acuerdo["Total Casos"] * 100
            ).round(2)
    ...
    with tab3:
        ...
        df_responsable.columns = [
            "Responsable",
            "Total Casos",
            "Eventos con Acuerdo",
            "Eventos sin Acuerdo",
            "Eventos Inasistencia",
            "Días Promedio",
        ]
        ...
        df_casos_resp = (
            df_filtrado.groupby(["responsable", "id_caso"])
            .agg({"tiene_acuerdo": "any"})
            .reset_index()
        )
        df_tasa_resp = (
            df_casos_resp.groupby("responsable")
            .agg({"id_caso": "count", "tiene_acuerdo": "sum"})
            .reset_index()
        )
        df_tasa_resp.columns = ["Responsable", "Total Casos", "Casos con Acuerdo"]
        df_tasa_resp["Tasa Éxito (%)"] = (
            df_tasa_resp["Casos con Acuerdo"] / df_tasa_resp["Total Casos"] * 100
        ).round(2)

        df_responsable = df_responsable.merge(
            df_tasa_resp[["Responsable", "Tasa Éxito (%)"]],
            on="Responsable",
            how="left",
        )
        ...
        if not df_responsable_eff.empty:
            ...
            df_responsable_eff["Días Promedio"] = pd.to_numeric(
                df_responsable_eff["Días Promedio"], errors="coerce"
            )
            df_responsable_eff = df_responsable_eff[
                df_responsable_eff["Días Promedio"].notna()
            ]
    ...
    with tab4:
        ...
        df_estado_temporal = (
            df_filtrado.groupby("id_caso")
            .agg({"estado_negocio": lambda x: x.iloc[0] if len(x) > 0 else ""})
            .reset_index()
        )

        df_casos_temporal = df_casos_temporal.merge(
            df_estado_temporal[["id_caso", "estado_negocio"]], on="id_caso", how="left"
        )
        ...
        mask_en_proceso = (
            (
                df_casos_temporal["estado_negocio"].str.contains(
                    "Vigente|Trámite", case=False, na=False
                )
            )
        ...
        df_casos_temporal.loc[df_casos_temporal["tiene_retiro"], "resultado"] = (
            "Retirado"
        )
        df_casos_temporal.loc[df_casos_temporal["tiene_inasistencia"], "resultado"] = (
            "Inasistencia"
        )
        df_casos_temporal.loc[df_casos_temporal["tiene_no_acuerdo"], "resultado"] = (
            "Sin Acuerdo"
        )
        df_casos_temporal.loc[df_casos_temporal["tiene_acuerdo"], "resultado"] = (
            "Con Acuerdo"
        )

        # Agrupar por resultado con conteo y tiempo promedio
        df_tiempo_resultado = (
            df_casos_temporal.groupby("resultado")
            .agg({"id_caso": "count", "dias_procesamiento": "mean"})
            .reset_index()
        )
        df_tiempo_resultado.columns = ["Resultado", "Cantidad", "Días Promedio"]
        df_tiempo_resultado = df_tiempo_resultado[
            df_tiempo_resultado["Días Promedio"].notna()
        ]
        df_tiempo_resultado["Días Promedio"] = df_tiempo_resultado[
            "Días Promedio"
        ].round(1)
        df_tiempo_resultado = df_tiempo_resultado.sort_values(
    ...
    with tab5:
        ...
        df_evento_estado = (
            df_filtrado.groupby(["evento", "estado"])
            .size()
            .reset_index(name="Cantidad")
        )
        df_evento_estado = df_evento_estado.sort_values(
            "Cantidad", ascending=False
        ).head(20)
